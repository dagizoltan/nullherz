...
### 1. Sequence Wrap Precision
The `SequencerProcessor` uses `f64` for beat positions, but relative sample calculation in split-blocks can accumulate tiny rounding errors over several hours of continuous playback.
- **Symptom**: Extremely subtle "drift" in long-duration performances.
- **Action**: Move to fixed-point or sample-absolute indexing for the sequencer timeline.

### 2. ALSA Descriptor Inheritance
