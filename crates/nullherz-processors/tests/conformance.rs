...
fn test_all_processors_conformance() {
    ...
    for (type_id, name) in processors {
        if let Some(mut processor) = registry.create_by_id(type_id, 0, 44100.0) {
            ...
            StabilityTester::verify_signal_bounds(proc_ref, 10).expect(&format!("Stability check failed for {}", name));

            // 2. Reset Determinism
            ConformanceSuite::verify_reset_consistency(proc_ref).expect(&format!("Reset consistency failed for {}", name));

            // 3. Sub-block Consistency (Phase-locking across automation boundaries)
            ConformanceSuite::verify_sub_block_consistency(proc_ref).expect(&format!("Sub-block consistency failed for {}", name));

            // 4. SIMD Alignment
            ConformanceSuite::verify_simd_alignment(proc_ref).expect(&format!("SIMD alignment check failed for {}", name));

            // 5. Parameter Bounds (NaN/Inf robustness)
            // We test the first 16 potential parameters
            for p_id in 0..16 {
                ConformanceSuite::verify_parameter_bounds(proc_ref, p_id).expect(&format!("Parameter robustness failed for {} on param {}", name, p_id));
            }
        }
    }
}
