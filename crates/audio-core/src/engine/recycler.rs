...
impl ResourceRecycler {
    ...
    pub fn recycle_bundle(
        ...
        if let Some(prod) = &mut self.bundle_garbage_producer {
            if let Err(b) = prod.push(bundle) {
                if let Some(overflow) = &mut self.bundle_overflow_producer {
        ...
        } else if let Some(overflow) = &mut self.bundle_overflow_producer {
