    ...
    pub fn update_peak_levels(
        ...
        for n_idx in 0..topo.node_count.min(crate::MAX_NODES) {
            ...
            for o_idx in 0..routing.output_count {
                let v_out = routing.output_indices.get(o_idx).copied().unwrap_or_default().index().min(crate::MAX_BUFFERS - 1);
                let p_idx = topo.virtual_to_physical.get(v_out).copied().unwrap_or_default().index().min(crate::MAX_BUFFERS - 1);
