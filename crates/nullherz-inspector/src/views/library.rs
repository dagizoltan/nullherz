...
fn render_track_inspector(app: &mut InspectorApp, ui: &mut Ui, _track_id: u64) {
    let theme = app.theme;
    if let Some(mut track) = app.library.cached_inspected_track.take() {
        ...
        if clear_clicked {
            ...
            app.library.cached_inspected_track = Some(track);
        }
    }
}
...
fn render_track_list(app: &mut InspectorApp, ui: &mut Ui) {
    ...
    // library scrolls at full frame rate. auto_shrink off: the list fills
    // the whole central region the surrounding panels leave it (this
    // function runs inside CentralPanel::show_inside, which owns every
    // pixel between the toolbar and the inspector).
    ScrollArea::vertical()
