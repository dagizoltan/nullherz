...
pub fn render(app: &mut InspectorApp, ui: &mut Ui) {
    let theme = app.theme;
    ui.heading(RichText::new("System Metrics & Telemetry").size(theme.type_heading));
        ...
        ui.vertical(|ui| {
            ...
            render_metric_group(ui, "DSP EXECUTION PLANE", frame_width, &theme, |ui| {
                if let Some(t) = &telemetry {
                    ...
                    ui.add_space(5.0);
                    ui.label(RichText::new("NODE PERFORMANCE BREAKDOWN").size(theme.type_caption).color(theme.text_secondary));
                    ...
                    for i in 0..64 {
                        ...
                        ui.painter().rect_filled(r, 0.0, theme.accent_muted);
                    }
                } else {
            ...
            render_metric_group(ui, "DNA LATENT SPACE PROJECTION", frame_width, &theme, |ui| {
                ...
                ui.add_space(5.0);
                ui.label(RichText::new("TIMBRAL TRAJECTORY").size(theme.type_caption).color(theme.text_secondary));
            });
            ...
            render_metric_group(ui, "DISTRIBUTED EXECUTION (REMOTE NODES)", frame_width, &theme, |ui| {
                if let Some(t) = &telemetry {
                    if t.remote_node_count == 0 {
                        ui.label(RichText::new("No remote nodes attached.").italics().size(theme.type_caption).color(theme.text_secondary));
                    } else {
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    for i in 0..worker_count {
                        ...
                        let color = if load > 0.9 {
                            ...
                            theme.text_disabled // Idle
                        };

                        ui.painter().rect_filled(r, theme.radius_sm, color);
                        ui.painter().text(r.center(), egui::Align2::CENTER_CENTER, format!("{}", i), egui::FontId::monospace(theme.type_caption), theme.text_primary);
                    }
                });
                ui.add_space(5.0);
                ui.label(RichText::new("STABLE PARALLEL STAGE EXECUTION").size(theme.type_caption).color(theme.text_disabled));
            });
        });
    });
}
...
{
    ui.label(RichText::new(title).size(theme.type_caption).strong().color(theme.text_secondary));
    Frame::none()
