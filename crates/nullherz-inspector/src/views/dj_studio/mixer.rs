    ...
    ui.vertical(|ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                let r_hi = widgets::render_knob(ui, &mut app.mixer.channel_eq_high[i], 0.0..=2.0, "HI", deck_color);
                if r_hi.changed() { changed = true; }
                let r_mid = widgets::render_knob(ui, &mut app.mixer.channel_eq_mid[i], 0.0..=2.0, "MID", deck_color);
                if r_mid.changed() { changed = true; }
                let r_low = widgets::render_knob(ui, &mut app.mixer.channel_eq_low[i], 0.0..=2.0, "LOW", deck_color);
            ...
            ui.vertical(|ui| {
                ...
                let filter_params = vec![
                    ("FLT", &mut app.mixer.channel_filter[i], nullherz_traits::DeckParamType::Filter, 0.0..=1.0),
                ];
        ...
        ui.horizontal(|ui| {
            let peak = app.viz.damped_peaks[i];
            ui.horizontal(|ui| {
                ui.spacing_mut().item_spacing.x = 2.0;
                widgets::render_vu_meter(ui, peak, app.mixer.channel_peak_hold[i], deck_color, 100.0);
                widgets::render_vu_meter(ui, peak, app.mixer.channel_peak_hold[i], deck_color, 100.0);
            });
            let r_fader = widgets::render_fader(ui, &mut app.mixer.channel_faders[i], 0.0..=1.0, deck_color, 100.0, 18.0);
            if r_fader.changed() {
