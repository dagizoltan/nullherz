...
pub fn render(app: &InspectorApp, ui: &mut Ui) {
    ...
    ScrollArea::vertical().id_source("ai_scroll").show(ui, |ui| {
        ui.vertical(|ui| {
            ...
            ui.add_space(theme.space_md);
            render_card_group(ui, "SYSTEM EVENTS", &theme, |ui| {
                ui.label("• Sidecar #1: Performance stable.");
                ui.label("• Cloud Sync: 4 new templates discovered.");
                ui.add_space(theme.space_xs);
                ui.label(RichText::new("Presenting simulated system events log").size(theme.type_caption).color(theme.text_disabled).italics());
            });
        });
    });
}
