...
- **Signal transparency at unity** (measured 2026-07-29, `conductor/examples/probe_signal_quality.rs`; 997 Hz, one deck, everything at unity, analyser floor 0.006% THD+N validated against a signal that never entered the console). Two defaults were colouring every signal, and neither was visible as a failure because both are *costs*, not errors:
  - **`SummingProcessor` shipped `soft_clip: true`.** `tanh()` is a WAVESHAPER, not a limiter — it distorts in proportion to level rather than waiting for a threshold, and it was on all **seven** summing nodes although the recorded intent was a "musical master" ceiling on the master bus alone. The console already has a real ceiling there (summing → `MASTERING_EQ` → **LIMITER** → out), so this was a second, undefeatable one in front of the first. Measured **THD+N 1.234% at −6 dBFS**; now **0.046%**, a 27× reduction, and level-independent (i.e. no longer a non-linearity at all). Studio converters sit at 0.001–0.01%; the audible threshold on music is ~0.1%. Param 1 re-enables it for anything that genuinely wants saturation.
  ...
  - Total gain error at unity went **−9.10 dB → −6.09 dB**, and console peak with four decks **0.5322 → 0.9656** — the mix had been running ~9 dB conservative. The remaining −6.02 dB is the master crossfader at centre with a LINEAR law, which is correct crossfader behaviour but see the note below.
