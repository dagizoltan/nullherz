...
### 0.0 `cargo test --workspace` hangs forever — **BLOCKING everything else on this list**
- *Location*: `crates/nullherz-conductor/src/sidecar_supervisor.rs:253-260` (fatal); `crates/nullherz-conductor/src/ptp_engine.rs:163` and `crates/nullherz-dna/src/network.rs:392` (thread leaks, same missing idea).
- *Detail*: `SidecarSupervisor::start_udp_return_listener` binds a **`std::net::UdpSocket`** — blocking, no read timeout, no `set_nonblocking` — and then calls `socket.recv_from()` in an unbounded loop **inside `tokio::spawn`**. The task occupies a tokio worker permanently and cannot be cancelled, so dropping the `Runtime` blocks forever. Any test that constructs a `Conductor` within a runtime context (`rt.enter()`, `#[tokio::test]`) hangs at teardown.
- *Evidence*: `async_hydration_test` hangs indefinitely — reproduced in isolation, 33 minutes elapsed for 11 seconds of CPU. `gdb` shows the test thread in `block_on(oneshot::Receiver)` and a `tokio-rt-worker` parked in `__libc_recvfrom` at `sidecar_supervisor.rs:260`. Two full-suite runs were killed by their own timeouts at the same point.
- *Impact*: the workspace suite has no completing run. With `--exclude nullherz-conductor` it is **287 passed, 0 failed** across 66 binaries, so nothing indicates the conductor's 113 tests are wrong — they are unverifiable. `scripts/verify.sh` (the only gate that runs at all, given §0 CI) is blocked by this too.
- *Fix*: make the listener non-blocking — `tokio::net::UdpSocket` and `.await`, or keep `std` with a read timeout and a shutdown flag checked each iteration. Give `Conductor` an explicit `shutdown()` that signals all three loops, and call it on `Drop`. The PTP loop already has a 200 ms read timeout, so it only needs the exit condition; the DNA listener needs the accept unblocked (a self-connect on shutdown, or a non-blocking listener).
