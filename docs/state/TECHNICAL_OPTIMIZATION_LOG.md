...
- **N2 peak-metering width** (scans all nodes incl. UI-invisible buses) and **#15 `push_command` Arc clone** — carried, low value.
- **#8 ALSA `eprintln!` on the audio thread — ✅ DONE (2026-07-23, stability).** Re-rated: low value as *perf*, but a real RT-safety hazard — `eprintln!` is a blocking `write(2)` under the stderr lock on the SCHED_FIFO callback, and the xrun-recovery log fired at the exact moment the device was already underrunning. Removed the periodic peak-log (a full peak scan + `eprintln`; engine telemetry already carries peaks) and replaced the recovery log with a lock-free `AtomicU64` xrun counter (`AlsaBackend::xruns()`) — zero blocking I/O on the RT path, plus queryable xrun visibility. Setup/teardown one-shot logs unchanged.
