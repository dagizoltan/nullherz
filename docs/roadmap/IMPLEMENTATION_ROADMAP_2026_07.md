...
| 1.4 | **Detection DONE; the WARNING is not wired.** `ipc_layer::cpu_governor()` and `cpufreq_driver()` exist and are careful in exactly the right way — the doc comment notes that `powersave` means opposite things under `acpi-cpufreq` (genuinely low P-state) and `intel_pstate` (the default, ramps under load), and that warning about the wrong one "trains people to dismiss the warning". But **the only callers are tests**: nothing in the conductor's boot path reads either, so no operator ever sees it. Finishing this is a few lines at startup, and it is the cheapest remaining item in the phase | S |
